        ...
        if (rooms) {
            for (let i = 0; i < rooms.length; i++) {
                const message = await Message.findOne(
                    { room: rooms[i]._id, admin: false },
                    {},
                    { sort: { created_at: -1 } }
                );
                const isLastMessageAdmin = await Message.findOne(
                    { room: rooms[i]._id },
                    {},
                    { sort: { created_at: -1 } }
                );
                rooms[i] = Object.assign({}, rooms[i], {
                    message: message ? message : {},
                    admin: isLastMessageAdmin ? isLastMessageAdmin.admin : true
                });
            }
            rooms.sort((room1, room2) => room2.message.created_at - room1.message.created_at);
            return rooms;
        }
