...
socketModule({ path: '/socket', transports: ['websocket'] });
...
app.use(express.static('_client/public/dist'));
/** Routes Definitions */
...
app.get('/*', (req, res) => {
    res.sendFile(__dirname + '/_client/public/dist/index.html');
})
...
io.on('connection', socket => {
    ...
    socket.on('newMessage', async data => {
        ...
        io.emit('updateRooms', JSON.stringify(rooms));
    });
    ...
    socket.on('roomAdded', async data => {
        io.emit('updateRooms', JSON.stringify(data));
    });
